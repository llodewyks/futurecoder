...
- When this README changes (or any other file under `progress-api/`), the workflow triggers automatically. For ad-hoc deployments use the `workflow_dispatch` action in GitHub or run `npm run build && ./functionapp-build.sh` locally to validate the bundle.
